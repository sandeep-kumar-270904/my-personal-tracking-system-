...
const SettingsPage = () => {
  ...
  const handleExportMd = async () => {
    try {
      toast.loading('Generating Markdown...', { id: 'md' });
      const response = await api.get('/data/md', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'placement_report.md');
      document.body.appendChild(link);
      link.click();
      toast.success('Markdown report generated', { id: 'md' });
    } catch (error) {
      toast.error('Failed to generate Markdown', { id: 'md' });
    }
  };
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-card p-6 md:p-8 rounded-2xl border border-white/5 h-full flex flex-col">
              ...
              <form onSubmit={handleSave} className="space-y-6 flex-1 flex flex-col">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">LeetCode Username</label>
                    <input type="text" className="w-full bg-[#13141f] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#00f0ff]" value={formData.leetcodeUsername} onChange={(e) => setFormData({...formData, leetcodeUsername: e.target.value})} placeholder="e.g. johndoe" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">GeeksforGeeks Username</label>
                    <input type="text" className="w-full bg-[#13141f] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#00f0ff]" value={formData.gfgUsername} onChange={(e) => setFormData({...formData, gfgUsername: e.target.value})} placeholder="e.g. johndoe" />
                  </div>
                </div>
          ...
          {activeTab === 'preferences' && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-card p-6 md:p-8 rounded-2xl border border-white/5 space-y-8 h-full">
              ...
              <div className="py-4 border-b border-white/5 space-y-4">
                <div>
                  <h3 className="font-bold text-white">Email & Platform Notifications</h3>
                  <p className="text-sm text-slate-400 mt-1">Granular control over what you receive.</p>
                </div>
                
                {[
                  { key: 'applicationUpdates', label: 'Application Updates', desc: 'Alerts when statuses change or deadlines approach.' },
                  { key: 'interviewReminders', label: 'Interview Reminders', desc: 'Nudges 24 hours and 1 hour before interviews.' },
                  { key: 'newResources', label: 'New Resources', desc: 'Weekly digest of new materials.' },
                  ...
                  <div key={key} className="flex items-center justify-between pl-4">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input 
                        type="checkbox" 
                        className="sr-only peer" 
                        checked={user?.notificationPreferences?.[key] ?? true} 
              ...
              <div className="py-4 border-b border-white/5 space-y-4">
                ...
                {user?.notificationPreferences?.whatsappAlerts && (
                  <div className="mt-4 p-4 bg-[#13141f] rounded-xl border border-white/10 flex flex-col md:flex-row gap-6 items-center">
                    <div className="w-32 h-32 bg-white rounded-xl p-2 flex shrink-0 items-center justify-center border-2 border-[#25D366]">
                      {/* Placeholder for real Twilio Sandbox QR Code */}
                      <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=whatsapp://send?phone=+14155238886&text=join%20team-studenttracker`} alt="Twilio QR Code" className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 space-y-3">
                      <h4 className="font-bold text-white text-sm">Activate WhatsApp Updates</h4>
                      <ol className="text-sm text-slate-400 space-y-1 list-decimal list-inside">
                        <li>Scan the QR code with your phone.</li>
                        <li>Send the pre-filled message <strong className="text-white">join team-studenttracker</strong> to our Twilio number.</li>
                        <li>Click "Check Activation" below to verify connection.</li>
                      </ol>
                      <button 
                        type="button"
                        onClick={async () => {
                          const loadToast = toast.loading('Checking Twilio activation status...');
                          try {
                            const res = await api.post('/auth/check-whatsapp');
                            toast.success(res.data.message || 'WhatsApp connected successfully!', { id: loadToast });
                          } catch (err) {
                            toast.error(err.response?.data?.message || 'Failed to verify connection.', { id: loadToast });
                          }
                        }}
                        className="px-4 py-2 bg-[#25D366]/20 hover:bg-[#25D366]/30 text-[#25D366] font-bold rounded-lg text-xs transition-colors border border-[#25D366]/50 inline-block mt-2"
                      >
                        Check Activation
                      </button>
                    </div>
                    ...
                    <div className="flex-1 w-full">
                      ...
                      <div className="flex flex-wrap gap-3">
                        ...
                        <button 
                          ...
                          className="px-4 py-2 bg-[#ff6b00]/20 hover:bg-[#ff6b00]/30 text-[#ff6b00] font-bold rounded-lg transition-colors border border-[#ff6b00]/30 flex items-center gap-2"
                        >
                          <Download className="w-4 h-4" /> Export MD Report
                        </button>
                      </div>
                    </div>
                  </div>
