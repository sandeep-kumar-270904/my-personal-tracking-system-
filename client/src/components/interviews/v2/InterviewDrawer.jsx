    ...
    <div className="space-y-6 pb-20">
      ...
      {!offerPrompt ? (
        ...
        <div className="bg-emerald-900/20 border border-emerald-500/30 p-5 rounded-xl text-center">
          <h4 className="font-bold text-emerald-400 mb-2 text-lg">🎉 Congratulations!</h4>
          <p className="text-sm text-emerald-200/80 mb-4">Add this as a tracked offer to compare compensation and sync deadlines.</p>
          <button 
            onClick={() => window.location.href = `/offers?add=true&company=${encodeURIComponent(interview.company)}&role=${encodeURIComponent(interview.role)}&sourceId=${interview._id}`}
            className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg mb-2"
          >
            Track Offer
          </button>
          <button onClick={() => { setOfferPrompt(false); onRefresh(); }} className="text-sm text-emerald-500 hover:text-emerald-400">Not yet / Skip</button>
        </div>
      )}
