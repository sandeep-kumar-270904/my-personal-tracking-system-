...
const CommandCenter = () => {
  ...
  const handleSelect = (type, item) => {
    ...
    const viewedItem = { type, id: item._id, title: item.title || item.company || item.name || 'Item', timestamp: Date.now() };
    const newRecent = [viewedItem, ...recentlyViewed.filter(i => i.id !== item._id)].slice(0, 5);
                ...
                {query.length > 0 && query.length < 2 ? (
                  ...
                  <div className="p-4">
                    {recentlyViewed.length > 0 && (
                      <>
                        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Recently Viewed</h3>
                        <div className="space-y-1">
                          {recentlyViewed.map((item) => (
                            <button
                              key={item.id}
                              onClick={() => handleSelect(item.type, { _id: item.id, ...item })}
