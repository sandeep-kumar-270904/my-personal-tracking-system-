...
const TableView = ({ applications, onAppClick, totalCount, page, limit, setSearchParams, hasEnoughDataForPrediction, prepSyllabuses = [] }) => {
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-white/5 text-xs uppercase tracking-wider text-slate-500 font-bold bg-white/[0.02]">
              ...
              <th className="p-4 font-bold text-slate-400">Prep Score</th>
          ...
          <tbody className="divide-y divide-white/5">
            {applications.map((app) => {
              ...
              const appSyllabus = prepSyllabuses.find(s => s.applicationId === app._id || s.company === app.company);
                  ...
                  <td className="p-4">
                    {appSyllabus ? (
                      <div className="relative group/score inline-block">
                        <div 
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold cursor-pointer transition-colors ${
                            appSyllabus.isCompleted 
                              ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20' 
                              : 'bg-[#ff6b00]/10 text-[#ff6b00] border border-[#ff6b00]/20 hover:bg-[#ff6b00]/20'
                          }`}
                          onClick={(e) => {
                            e.stopPropagation();
                            window.location.href = `/prephub?syllabusId=${appSyllabus._id}`;
                          }}
                        >
                          {appSyllabus.isCompleted ? '100%' : `${appSyllabus.progress || Math.round((appSyllabus.completedTopics?.length || 0) / Math.max(appSyllabus.topics?.length || 1, 1) * 100)}%`}
                        </div>
                        
                        {/* Mini-modal popover on hover */}
                        <div className="absolute z-50 left-1/2 -translate-x-1/2 bottom-full mb-2 w-64 bg-[#1a1b26] border border-white/10 rounded-xl shadow-2xl opacity-0 invisible group-hover/score:opacity-100 group-hover/score:visible transition-all p-4 pointer-events-none">
                          <div className="text-white text-sm font-bold mb-2">Prep Syllabus</div>
                          ...
                          <div className="w-full bg-white/5 rounded-full h-1.5 mb-3 overflow-hidden">
                            <div 
                              className={`h-1.5 rounded-full ${appSyllabus.isCompleted ? 'bg-emerald-500' : 'bg-[#ff6b00]'}`} 
                              style={{ width: `${appSyllabus.isCompleted ? 100 : (appSyllabus.progress || Math.round((appSyllabus.completedTopics?.length || 0) / Math.max(appSyllabus.topics?.length || 1, 1) * 100))}%` }}
                            ></div>
                          </div>
                          <div className="text-xs text-slate-400 mb-2">
                            {appSyllabus.topics?.length || 0} Topics | {appSyllabus.resources?.length || 0} Resources
                          </div>
                          <div className="text-xs text-[#ff6b00] font-medium flex items-center">
