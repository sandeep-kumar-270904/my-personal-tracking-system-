...
exports.exportMd = async (req, res) => {
  try {
    const apps = await Application.find({ userId: req.user._id }).lean();
    const offers = await Offer.find({ userId: req.user._id }).lean();
    
    let mdContent = `# StudentTracker Placement Report\n\n`;
    mdContent += `Generated on: ${new Date().toLocaleDateString()}\n\n`;
    mdContent += `## Summary\n`;
    mdContent += `- **Total Applications**: ${apps.length}\n`;
    mdContent += `- **Total Offers**: ${offers.length}\n\n`;
    mdContent += `## Applications Timeline\n\n`;
    mdContent += `| Company | Role | Status | Date Applied |\n`;
    mdContent += `| ------- | ---- | ------ | ------------ |\n`;
    
    apps.forEach(app => {
      mdContent += `| ${app.company || 'Unknown'} | ${app.role || 'Unknown'} | ${app.status || 'Applied'} | ${new Date(app.appliedDate || Date.now()).toLocaleDateString()} |\n`;
    });
    
    res.setHeader('Content-Type', 'text/markdown');
    res.setHeader('Content-Disposition', 'attachment; filename=placement_report.md');
    res.send(mdContent);
  } catch (err) {
    console.error(err);
