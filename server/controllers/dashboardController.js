...
const getDashboardStats = async (req, res) => {
  try {
    ...
    // Offers Received and Nearest Deadline
    const offersReceived = await Offer.countDocuments({ userId });
    const pendingOffers = await Offer.find({ userId, status: 'pending_decision', decision_deadline: { $ne: null } })
      .sort({ decision_deadline: 1 })
      .limit(1);
    const nearestOfferDeadline = pendingOffers.length > 0 ? pendingOffers[0].decision_deadline : null;
