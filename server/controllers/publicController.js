...
exports.getSharedOffers = async (req, res) => {
  try {
    const { token } = req.params;
    const user = await User.findOne({ 
      $or: [
    ...
    if (!user) {
      return res.status(404).json({ message: 'Shared link not found or has been revoked.' });
    }

    const offers = await Offer.find({ userId: user._id }).sort('-createdAt');

    // Strip private info
    const publicOffers = offers.map(o => {
      const p = o.toObject();
      delete p.notes;
      delete p.negotiationLog;
      return p;
    });
    ...
    console.error('Error fetching shared offers:', error);
    res.status(500).json({ message: 'Server error fetching shared offers' });
  }
};
