...
exports.archiveActiveGoals = async (req, res) => {
  try {
    ...
    const result = await Goal.updateMany(
      ...
      { $set: { status: 'archived', archived_by_offer_id: offerId || null } }
    );
...
// @desc    Reactivate goals archived by an offer
// @route   POST /api/goals/reactivate-for-offer/:offerId
exports.reactivateForOffer = async (req, res) => {
  try {
    const result = await Goal.updateMany(
      { user_id: req.user.id, status: 'archived', archived_by_offer_id: req.params.offerId },
      { $set: { status: 'active', archived_by_offer_id: null } }
    );
    res.json({ message: 'Goals reactivated successfully', count: result.modifiedCount });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
