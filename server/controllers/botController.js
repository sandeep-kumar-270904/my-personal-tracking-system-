...
- "LOG_OFFER": if they received a job offer. Extract "company" and "ctc" (a number, if specified).
...
{"intent": "LOG_OFFER", "company": "Amazon", "ctc": 1500000}
`;
      ...
      } else if (intent === 'LOG_OFFER' && company) {
        const Offer = require('../models/Offer');
        const offer = await Offer.create({
          userId: user._id,
          company_name: company,
          role_title: 'Unknown Role (WhatsApp)',
          status: 'pending_decision',
          offer_received_date: new Date(),
          ctc_annual: parsed.ctc || null,
          offer_type: 'full_time'
        });
        responseMessage = `🎉 Amazing news! I've logged an offer from ${company}${parsed.ctc ? ` for ${parsed.ctc}` : ''}. Don't forget to add the full details in the dashboard!`;
